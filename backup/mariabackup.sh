...
INNOBACKUPEX=$(which mariabackup)
[ -f "$INNOBACKUPEX" ] || die "mariabackup script not found - please ensure mariabackup is installed before proceeding."
...
full_backup () {
        ...
        mkdir -p  "$FULLS_DIRECTORY"/"$foldername"
        $INNOBACKUPEX_COMMAND --defaults-file=$PATH_TO_MYSQL_CONFIG --backup --target-dir="$FULLS_DIRECTORY"/"$foldername" --user="$MYSQL_USER" --password="$MYSQL_PASS"
...
incremental_backup () {
        ...
        incfoldername=$(date +%Y-%m-%d_%H-%M-%S)
        mkdir -p  "$INCREMENTALS_DIRECTORY"/"$incfoldername"
        $INNOBACKUPEX_COMMAND --backup --incremental-basedir="$LAST_BACKUP" --target-dir="$INCREMENTALS_DIRECTORY"/"$incfoldername" --user="$MYSQL_USER" --password="$MYSQL_PASS"
...
if [ "$1" = "full" ]; then
        ...
        if [[ -d "$BACKUP" ]]; then
                ...
                if [[ "$BACKUP" == *full* ]]; then
                        ...
                        $INNOBACKUPEX_COMMAND --prepare --apply-log-only --target-dir $DESTINATION  &>> $LOG_FILE || fail
                        echo -e "...done.\n"
                else
      ...
      $INNOBACKUPEX_COMMAND --prepare --apply-log-only --target-dir=$DESTINATION &>> $LOG_FILE || fail
        ...
        $INNOBACKUPEX_COMMAND --prepare --apply-log-only --target-dir=$DESTINATION --incremental-dir=$INCREMENTAL &>> $LOG_FILE || fail
                                echo -e "...done.\n"
                        done

                        echo "Finalising the destination..."
                        #$XTRABACKUP_COMMAND --prepare --target-dir=$DESTINATION  &>> $LOG_FILE || fail
      $INNOBACKUPEX_COMMAND --prepare --target-dir=$DESTINATION &>> $LOG_FILE || fail
                        echo -e "...done.\n"
                fi
