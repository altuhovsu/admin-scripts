...
echo "Full backup started on `hostname` at `date +"%Y-%m-%d %H:%M:%S"`." > $tmp_mail
echo "Output follows" >> $tmp_mail
echo >> $tmp_mail

/root/backup/xtrabackup.sh full >> /var/log/db-backup.log 2>&1
/root/backup/duplicity.sh full >> /var/log/backup.log 2>&1
...
echo "Full backup ended on `hostname` at `date +"%Y-%m-%d %H:%M:%S"`." >> $tmp_mail
...
echo "From: $mailfrom";
echo "To: $mailto";
echo "Subject: "Full backup ended on `hostname` at `date +"%Y-%m-%d %H:%M:%S"`."";
